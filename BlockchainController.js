...
class BlockchainController {
    ...
    // This endpoint allows you to check validity of entire chain
    getChainValidity() {
        this.app.get("/validateChain", async (req, res) => {
            try {
                let errorList = await this.blockchain.validateChain();
                if (!errorList.length) {
                    res.status(200).send({message: "Chain is valid"});
                } else {
                    res.status(200).send({message: "Chain isn't valid", errors: errorList})
                }
            } catch (error) {
                return res.status(500).send("An error happened!");
            } 
